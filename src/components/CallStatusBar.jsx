const statusStyles = {
  connecting: 'bg-amber-500/20 text-amber-200 border-amber-400/40',
...
const CallStatusBar = ({ statusLabel, tone = 'idle' }) => {
  ...
  return (
    <header className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur-lg">
      ...
      <span className={`rounded-full border px-3 py-1 text-xs font-semibold uppercase tracking-wide ${toneClass}`}>
        {pillLabel}
      </span>
    </header>
  );
